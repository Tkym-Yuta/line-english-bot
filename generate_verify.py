...
OUTPUT_DIR = Path("outputs_verify")
TEST_NAME = f"test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

# 新規問題の範囲指定（必要に応じて変更）
RANGES_CONFIG = [
    {"start": 1, "end": 1000, "count": 8},
    {"start": 1001, "end": 1900, "count": 8},
]
...
def build_answer(jp_to_en, en_to_jp):
    ...
    lines.append("【A】\n")
    for i, w in enumerate(en_to_jp, 1):
        lines.append(f"{i}. [{w['number']}] {w['word']} = {w['meaning']}")

    lines.append("\n【B】\n")
    for i, w in enumerate(jp_to_en, len(en_to_jp) + 1):
        lines.append(f"{i}. [{w['number']}] {w['meaning']} = {w['word']}")
